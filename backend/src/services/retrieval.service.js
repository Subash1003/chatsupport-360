...
//
// Phase 7: gather the trusted context for a chat answer.
//
//   1. Embed the query.
//   2. Vector search Qdrant with an IDENTITY-SCOPED filter built server-side:
//        anonymous      -> visibility = "public"
//        authenticated  -> public  OR  (private AND customer_id = <from JWT>)
//      customerId comes only from req.customer.customer_id (spec rule 1).
//   3. For an authenticated caller, also pull a small LIVE snapshot from MySQL
//      (the source of truth) so answers about their own projects/tickets/subs
//      are fresh even if the vector store is behind.
//   4. Drop weak hits (below RAG_MIN_SCORE).
//
// Retrieval failures degrade to "no context" — the chat still answers (with the
...
async function vectorContext(query, customerId, { boostOffers = false } = {}) {
  ...
  try {
    hits = await search({
      vector,
      filter: filterForIdentity(customerId),
      limit: env.RAG_TOP_K,
    });

    // Phase 8 routing: an OFFER_OR_PROMOTION query also pulls offer docs
    // directly, so live offers surface even when generic scoring buries them.
    if (boostOffers) {
...
async function accountSnapshot(customerId) {
  try {
    ...
    // Nothing at all (token valid but the row is gone) — no snapshot.
...
/**
 * @param {{ query: string, customerId: string|null, classification?: string }} args
 * @returns {Promise<{ contextBlocks: string[], sources: object[], grounded: boolean }>}
 */
export async function retrieveContext({ query, customerId, classification }) {
  // Phase 8 routing. Security is unaffected — the identity filter is unchanged.
